...
import it.sevenbits.cards.core.repository.AccountRepository;
...
public class AccountService {
    ...
    @Autowired
    private AccountRepository activationRepository;
    ...
    public Account generateActivationHash(RegistrationForm form) throws ServiceException {
        Account account = new Account();
        account.setEmail(form.getEmail());
        try {
            account.setHash(Sha.hash256());
        } catch (NoSuchAlgorithmException e) {
            e.printStackTrace();
        }
        String hash = null;
        try{
            activationRepository.findHashByEmail(account.getEmail());
        }catch (Exception e){
            throw new ServiceException("An error occurred while finding hash by email: " + e.getMessage(), e);
        }
        if (hash == null) {
            try {
                activationRepository.save(account);
            }catch (Exception e){
                throw new ServiceException("An error occurred while saving account activation: " + e.getMessage(), e);
            }
        } else {
            try {
                activationRepository.updateHash(account.getHash(), account.getEmail());
            }catch (Exception e){
    ...
    @Async
    public void sendEmail(Account account) throws ServiceException {
        if (account == null) {
